...
import type { ExposureBasis, ExposureRow, SourceLocation } from "@/data/gl-exposure";
import {
  basisLabel,
  cityFor,
  classKey,
...
const sourceData = () => ({
  rows: seedExposureRows.map((row) => ({ ...row })),
...
export function GlExposureWorkbench() {
  ...
  const attentionCount = exposureRows.filter(needsAttention).length;

  const updateRow = (rowId: string, patch: Partial<ExposureRow>) => {
    setExposureRows((current) =>
      current.map((row) => {
        if (row.id !== rowId) return row;
        const classCode = patch.classCode ?? row.classCode;
        return {
          ...row,
          ...patch,
  ...
  const drillToDetail = (rows: ExposureRow[], label: string, attentionOnly = false) => {
    const scopedRows = attentionOnly ? rows.filter(needsAttention) : rows;
  ...
  const addExposure = () => {
    ...
    const classCode = draft.classCode;
    const issueType = classCode ? (draft.amount === null ? "blank_exposure" : "") : "missing_class";
    const issueLabelValue =
      issueType === "missing_class" ? "Missing class" : issueType === "blank_exposure" ? "Blank exposure" : "";
    const row: ExposureRow = {
      ...
      status: issueType ? "review" : "included",
      issueType,
      issueLabel: issueLabelValue
    };
  ...
  const downloadCsv = () => {
    ...
    setDownloadNotice("CSV download simulated: gl-exposures.csv");
    ...
    <main className="page-shell">
        ...
        <header className="workbench-header">
          <div>
            ...
            <div className="header-subline">
              <Tag minimal>{formatNumber(includedRows.length)} included rows</Tag>
              ...
              {activeView === "analysis" ? (
                <div className="control-cluster">
                  <FormGroup label="Group by" className="compact-control">
                    <HTMLSelect value={groupBy} onChange={(event) => setGroupBy(event.currentTarget.value as GroupBy)}>
                  ...
                  {groupBy === "state" && (
                    <FormGroup label="Measure" className="compact-control">
                      <HTMLSelect
                        value={measure}
                ...
                <div className="control-cluster detail-control-cluster">
                  {detailScope && (
                    <Tag
                      className="scope-tag"
                      minimal
                      onRemove={() => setDetailScope(null)}
                    >
                      {detailScope.label}
                    </Tag>
                  )}
                  <FormGroup label="Show" className="compact-control">
                    <HTMLSelect
                      value={detailFilter}
            ...
            <section className="table-zone" aria-label={activeView === "analysis" ? "Analysis table" : "Detail table"}>
              {activeView === "analysis" ? (
                ...
                <DetailTable
                  ...
                  onRestoreRow={(rowId) => updateRow(rowId, { status: "included", issueType: "", issueLabel: "" })}
      ...
      <Dialog isOpen={addOpen} title="Add exposure" onClose={() => setAddOpen(false)}>
        <DialogBody>
          ...
          <Callout intent="warning" className="dialog-callout">
        ...
        <DialogFooter
          actions={
            <>
              ...
              <Button intent="primary" text="Save" onClick={addExposure} />
            </>
          }
        />
      </Dialog>
      ...
      <Alert
        ...
        onConfirm={() => {
          if (deleteTarget) setExposureRows((current) => current.filter((row) => row.id !== deleteTarget.id));
...
function getDetailRows(
  ...
  return rows.filter((row) => {
    ...
    if (detailFilter === "attention") return needsAttention(row);
    if (detailFilter === "excluded") return row.status === "excluded";
    if (detailFilter === "unknown_state") return row.state === "Unknown" || row.issueType === "unknown_state";
...
function buildRollups(
  ...
  const rollups: Rollup[] = [...groups.entries()].map(([key, group]) => {
    ...
    return {
      ...
      attention: group.filter(needsAttention).length
    };
  });
      ...
      <table className={`work-table ${Classes.HTML_TABLE} ${Classes.HTML_TABLE_STRIPED}`}>
        <thead>
          <tr>
            ...
            <th className="status-col"><SortButton label="" keyName="attention" align="center" activeSort={sort} onSort={onSort} /></th>
          </tr>
        </thead>
        <tbody>
          {rollups.length ? (
...
function RollupIssueButton({
  ...
  const title = `${attentionRows.length} row${attentionRows.length === 1 ? "" : "s"} need review: ${[
    ...new Set(attentionRows.map(issueLabel).filter(Boolean))
  ].join(", ")}`;
  return (
    ...
    <table className={`work-table detail-table ${Classes.HTML_TABLE} ${Classes.HTML_TABLE_STRIPED}`}>
      <thead>
        <tr>
          ...
          <th className="status-col"><SortButton label="" keyName="issueSort" align="center" activeSort={sort} onSort={onSort} /></th>
      ...
      <tbody>
        {rows.length ? (
          rows.map((row) => {
            ...
            return (
              <tr key={row.id} className={row.status === "excluded" ? "excluded-row" : ""}>
                ...
                <td className="status-col"><IssueTag row={row} compact /></td>
                {editMode && (
                  <td className="actions-col">
                    <ButtonGroup>
                      {row.status === "excluded" ? (
                        <Tooltip content="Restore row">
                          <Button small icon="undo" aria-label={`Restore ${row.id}`} onClick={() => onRestoreRow(row.id)} />
                        </Tooltip>
                      ) : (
          ...
          <EmptyRow span={editMode ? 8 : 7} label="No rows found" />
        )}
      </tbody>
    </table>
  );
...
function FilterPopover({
  ...
  return (
    ...
    >
      <Button icon="filter" text={selectedCount ? `Filter ${selectedCount}` : "Filter"} />
    </Popover>
  );
}

function IssueTag({ row, compact = false }: { row: ExposureRow; compact?: boolean }) {
  const label = issueLabel(row);
  if (!label) return null;
  const intent = row.status === "excluded" || isCriticalIssue(row) ? "danger" : "warning";
  return (
    <Tooltip content={issueText(row)}>
      <Tag minimal={compact} intent={intent} icon={row.status === "excluded" ? "disable" : "warning-sign"}>
        {compact ? "" : label}
      </Tag>
    </Tooltip>
  );
...
function confidenceForRow(row: ExposureRow) {
  ...
  if (row.status === "review" || row.sourceType === "Extractor Draft") return "Low confidence";
